...
const TESTNET = process.env.BYBIT_TESTNET === 'true'; // Set to false for mainnet by default
...
export async function getPositions() {
  ...
  const positions = response.result.list.map((pos: any) => ({
    id: pos.positionIdx.toString() as string,
    symbol: pos.symbol as string,
    side: pos.side === 'Buy' ? 'BUY' : ('SELL' as 'BUY' | 'SELL'), // Map Bybit's 'Buy'/'Sell' to 'BUY'/'SELL'
    ...
    status: parseFloat(pos.size) > 0 ? 'OPEN' : ('CLOSED' as 'OPEN' | 'CLOSED'),
    openTime: new Date(parseInt(pos.createdTime)).toISOString(),
    closeTime: pos.updatedTime && parseFloat(pos.size) === 0 ? new Date(parseInt(pos.updatedTime)).toISOString() : undefined,
...
// Execute trade
export async function executeTrade(trade: { symbol: string; side: 'BUY' | 'SELL'; size: number; type: 'market' | 'limit'; price?: number }) {
  const params: any = {
    category: 'linear',
    symbol: trade.symbol,
    side: trade.side === 'BUY' ? 'Buy' : 'Sell', // Convert to Bybit's casing
  ...
  await getPositions(); // Update positions
