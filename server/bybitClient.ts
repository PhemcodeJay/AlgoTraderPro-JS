...
// Default to mainnet unless explicitly TESTNET=true
...
export async function getPositions(): Promise<Position[]> {
  ...
  const positions: Position[] = response.result.list.map((pos: any) => {
    const side = (pos.side === 'Buy' ? 'BUY' : 'SELL') as 'BUY' | 'SELL';
...
// --- Generate trading signals (real logic, not random) ---
...
export async function scanSignals(): Promise<Signal[]> {
  ...
  for (const symbol of symbols) {
    ...
    const closes = kline.result.list
      .map((c: any) => parseFloat(c[4])) // close price is index 4
      .reverse();
    ...
    if (type) {
      signals.push({
        ...
        symbol,
        type,
        score: Math.abs(shortMA - longMA), // strength of signal
...
export async function executeTrade(trade: {
  ...
  await getPositions(); // refresh positions after trade
