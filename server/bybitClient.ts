...
import { storage, type Position, type MarketData, type Balance } from './storage';
...
export function initWebSocket() {
  ...
  bybitWsClient.on('reconnect', () => console.log('[Bybit WS] Reconnecting...'));
  (bybitWsClient as any).on('error', (err: any) => {
    const msg = err?.message || err?.toString() || 'Unknown error';
    console.error('[Bybit WS] Error:', msg);
  });
...
export async function getBalance(): Promise<Balance> {
  try {
    ...
    // FIX: Cast to any to handle the union type from Bybit API
    const usdt: any = response.result.list[0]?.coin?.find((c: any) => c.coin === 'USDT') ||
      response.result.list.find((asset: any) => asset.coin === 'USDT');
...
export async function getMarketData(symbols?: string[]): Promise<MarketData[]> {
  ...
  try {
    ...
    console.warn('[Bybit] getMarketData API error:', err.message);
  }

  // FIX: Return empty array instead of mock data with hardcoded prices
  return [];
}
...
export async function executeTrade(trade: any, mode?: 'virtual' | 'real') {
  ...
  // ALWAYS use live market price for both Virtual and Real modes
  ...
  try {
    ...
    console.warn(`[Bybit] Failed to fetch price for ${trade.symbol}: ${err.message}`);
  }

  if (!currentPrice) currentPrice = trade.price || 0;

  if (m === 'virtual') {
    const tradingConfig = await storage.getTradingConfig();
    const leverage = trade.leverage || tradingConfig.leverage;
    ...
    // FIX: Update balance to reflect margin used
    const balance = await storage.getBalance();
    const marginUsed = (currentPrice * trade.size) / leverage;
    await storage.setBalance({
      capital: balance.capital,
      available: Math.max(0, balance.available - marginUsed),
      used: balance.used + marginUsed,
    });
  ...
  // FIX: Set leverage separately before placing order (Bybit API requires this)
  if (trade.leverage) {
    try {
      await (bybitRestClient as any).setLeverage({
        category: 'linear',
        symbol: trade.symbol,
        leverage: trade.leverage.toString(),
      });
    } catch (err: any) {
      console.warn(`[Bybit] Failed to set leverage for ${trade.symbol}: ${err.message}`);
    }
  }
...
export async function closePosition(positionId: string, mode?: 'virtual' | 'real') {
  ...
  if (m === 'virtual') {
    ...
    // FIX: Calculate margin used based on actual entry price, size, and leverage
    const margin = (pos.entryPrice * pos.size) / (pos.leverage || 10);

    // FIX: Return margin to available balance and add PnL
    await storage.setBalance({
    ...
    // FIX: Archive the closed position as a trade for performance tracking
    await storage.archiveTrade(pos);

    await storage.removePosition(positionId);
    return { success: true, pnl: pos.pnl, pnlPercent: pos.pnlPercent };
  }
