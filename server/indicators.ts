...
export function EMA(data: number[], period: number): number[] {
  if (data.length < period) return new Array(data.length).fill(0);
  const k = 2 / (period + 1);
  const smaInitial = SMA(data, period);
  const ema: number[] = [];
  for (let i = 0; i < data.length; i++) {
    if (i < period - 1) {
      ema.push(0);
    } else if (i === period - 1) {
      ema.push(smaInitial[i]);
    } else {
...
export function scoreSignal(closes: number[], highs: number[], lows: number[], volumes: number[]): EnhancedSignalScore {
  ...
  if (macdHist > 0.01) {
    buyScore += 8;
    signals.push("MACD_BULLISH_HIST");
  } else if (macdHist < -0.01) {
    sellScore += 8;
    signals.push("MACD_BEARISH_HIST");
  }
  ...
  if (price <= bbLower && bbLower > 0) {
    buyScore += 15;
    signals.push("BB_OVERSOLD");
  } else if (price >= bbUpper && bbUpper > 0) {
    sellScore += 15;
  ...
  if (volumeRatio > 2) {
    if (price > (indicators.sma20[indicators.sma20.length - 1] || price)) {
      buyScore += 12;
      signals.push("VOLUME_VERY_HIGH_BULL");
    } else {
      sellScore += 12;
      signals.push("VOLUME_VERY_HIGH_BEAR");
    }
  } else if (volumeRatio > 1.5) {
    if (price > (indicators.sma20[indicators.sma20.length - 1] || price)) {
      buyScore += 6;
      signals.push("VOLUME_HIGH_BULL");
    } else {
      sellScore += 6;
      signals.push("VOLUME_HIGH_BEAR");
    }
  }
  ...
  if (indicators.sma20.length > 1 && indicators.sma50.length > 1) {
    ...
    if (currentSMA20 > prevSMA20) {
      buyScore += 10;
      signals.push("SMA20_UPTREND");
    } else {
      sellScore += 10;
      signals.push("SMA20_DOWNTREND");
    }
  }
