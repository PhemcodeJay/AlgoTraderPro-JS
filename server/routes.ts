...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get('/api/notification-config', async (_req, res) => res.json(await storage.getNotificationConfig()));
  ...
  app.post('/api/trading-mode', async (req, res) => {
    const { mode } = req.body;
    const status = await storage.getAppStatus();
    await storage.setAppStatus({ ...status, tradingMode: mode });
    res.json({ success: true });
  });
