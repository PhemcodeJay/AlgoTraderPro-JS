import { storage, type Signal, type MarketData } from './storage';
    ...
    try {
      ...
      try {
        ...
        if (!kline?.result?.list || kline.retCode !== 0) {
          console.warn(`[scanSignals] No kline data for ${symbol}, skipping`);
          return null;
        }
      } catch (err: any) {
        console.warn(`[scanSignals] Data fetch failed for ${symbol}, skipping`);
        return null;
      }
      ...
      // Validate data quality - skip if all prices are the same (stale data)
      const uniquePrices = new Set(closes).size;
      if (uniquePrices < 5) {
        console.warn(`[scanSignals] Insufficient price variation for ${symbol}, skipping`);
        return null;
      }
      ...
      // FIX: Require a minimum net score difference to avoid weak signals
      // A signal with buyScore=55 and sellScore=54 is effectively neutral
      const netScoreDiff = Math.abs(scores.buyScore - scores.sellScore);
      if (netScoreDiff < 15) {
        console.warn(`[scanSignals] ${symbol}: Net score diff too small (${netScoreDiff.toFixed(1)}), skipping weak signal`);
        return null;
      }
