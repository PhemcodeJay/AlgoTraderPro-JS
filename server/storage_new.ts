...
import { UserType, InsertUser } from '../shared/schema.ts';
import { fileURLToPath } from 'url';
import { IndicatorData } from './indicators.ts';
...
export interface IStorage {
  ...
  archiveTrade(position: Position): Promise<void>;
  getTradeHistory(): Promise<Position[]>;
...
export class SqliteStorage implements IStorage {
  ...
  async setSignals(signals: Signal[]): Promise<void> {
    // FIX: Only clear signals for the same interval, not ALL signals
    if (signals.length > 0) {
      const interval = signals[0].interval;
      await this.db.run('DELETE FROM signals WHERE json_extract(data, \'$.interval\') = ?', interval);
    }
  ...
  // FIX: Archive closed position as a trade for performance tracking
  async archiveTrade(position: Position): Promise<void> {
    const trade: Position = {
      ...position,
      status: 'CLOSED',
      closeTime: new Date().toISOString(),
    };
    await this.db.run('INSERT OR REPLACE INTO trades (id, data) VALUES (?, ?)',
      trade.id, JSON.stringify(trade));
  }

  // FIX: Get trade history for performance analysis
  async getTradeHistory(): Promise<Position[]> {
    const rows = await this.db.all('SELECT data FROM trades ORDER BY closeTime DESC LIMIT 100');
    return rows.map((r: any) => JSON.parse(r.data));
  }

  // FIX: Calculate performance metrics from trade history
